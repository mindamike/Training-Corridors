...
""")
    ...
    # ── Derivation of μ* = 4 (parameter-free) ───────────────────────────────
    # The corridor crossover point μ* is defined by ΔV_SN = ΔV_TC:
    #
    #   4μ^{3/2}/3  =  μ³/6
    #   8μ^{3/2}    =  μ³
    #   8           =  μ^{3/2}
    #   μ*          =  8^{2/3}  =  4   (exact)
    #
    # This uses NO free parameters — only the two canonical normal forms
    # ẋ = μx − x² (TC) and ẋ = μ − x² (SN) with unit coefficients.
    # The normal-form coefficients are fixed by the classification theorem
    # (codimension-1, leading-order), so μ* = 4 is a structural constant
    # of the RST framework, not a parameterization choice.
    #
    # Verification:
    mu_star = 8.0 ** (2.0 / 3.0)
    dv_tc_at_star = mu_star**3 / 6
    dv_sn_at_star = 4 * mu_star**1.5 / 3
    ratio_at_star = dv_sn_at_star / dv_tc_at_star

    print(f"""
  Derivation of μ* (crossover point where ΔV_SN = ΔV_TC):
    Set  4μ^{{3/2}}/3  =  μ³/6
    →    8μ^{{3/2}}   =  μ³
    →    μ*           =  8^{{2/3}}  =  {mu_star:.6f}  (exact: 4)

  Verification at μ* = {mu_star:.4f}:
    ΔV_TC = μ*³/6         = {dv_tc_at_star:.6f}
    ΔV_SN = 4μ*^{{3/2}}/3 = {dv_sn_at_star:.6f}
    Ratio = {ratio_at_star:.8f}  (should be exactly 1.0)""")

    assert abs(ratio_at_star - 1.0) < 1e-10, f"μ* check failed: ratio={ratio_at_star}"
    print("  ✓ μ* = 4 confirmed — parameter-free, from canonical normal forms.\n")
    print("""  Note on universality: μ* = 4 holds for unit-coefficient normal forms.
  Real systems with non-unit leading coefficients (a in ẋ=μx−ax², b in ẋ=μ−bx²)
  give μ* = (8b/a)^{2/3}·(some power of b/a). The EXISTENCE of a crossover
  and the structural argument are universal; the exact value 4 is canonical-form
  specific. This is stated explicitly so the result is not overclaimed.
""")

    RESULTS["asymmetry"] = {"formula": "8/mu^1.5", "mu_star": mu_star,
                            "mu_star_exact": 4.0, "derivation": "8^(2/3)"}

    if show_plots:
        mu_p = np.linspace(0.05, 5.0, 400)
        ratio_p = 8.0 / mu_p**1.5
        fig, axes = plt.subplots(1, 2, figsize=(11, 4))

        ax = axes[0]
        ax.semilogy(mu_p, ratio_p, lw=2, color="C3")
        ax.axhline(y=1, color="gray", ls="--", label="Equal barriers  (ΔV_SN = ΔV_TC)")
        ax.axvline(x=4.0, color="C2", ls=":", lw=1.8,
                   label=r"$\mu^* = 4$  (crossover, exact)")
        ax.fill_between(mu_p[mu_p < 4], ratio_p[mu_p < 4], 1,
                        alpha=0.15, color="C3", label="SN dominates (μ < 4)")
        ax.scatter([4.0], [1.0], s=80, color="C2", zorder=5)
        ax.set_xlabel(r"$\mu$")
        ax.set_ylabel(r"$\Delta V_{SN} / \Delta V_{TC}$")
        ax.set_title(r"Corridor Asymmetry: $\Delta V_{SN}/\Delta V_{TC} = 8/\mu^{3/2}$")
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)

        ax = axes[1]
        # Show the derivation of μ* = 4 graphically:
        # plot both barrier heights vs μ and mark the crossing
        mu_d = np.linspace(0.1, 6.0, 400)
        dv_tc = mu_d**3 / 6
        dv_sn = 4 * mu_d**1.5 / 3
        ax.loglog(mu_d, dv_tc, lw=2, color="C0",
                  label=r"$\Delta V_{TC} = \mu^3/6$")
        ax.loglog(mu_d, dv_sn, lw=2, color="C3",
                  label=r"$\Delta V_{SN} = 4\mu^{3/2}/3$")
        ax.axvline(x=4.0, color="C2", ls=":", lw=1.8)
        ax.scatter([4.0], [4.0**3 / 6], s=80, color="C2", zorder=5,
                   label=r"$\mu^* = 8^{2/3} = 4$ (ΔV_SN = ΔV_TC)")
        ax.set_xlabel(r"$\mu$")
        ax.set_ylabel(r"Barrier height $\Delta V$")
        ax.set_title(r"$\mu^* = 4$ from $\Delta V_{SN} = \Delta V_{TC}$  [parameter-free]")
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)
        # Annotate derivation steps
        ax.text(0.04, 0.30,
                r"$4\mu^{3/2}/3 = \mu^3/6$" + "\n"
                r"$\Rightarrow\;\mu^* = 8^{2/3} = 4$",
...
""")
    ...
    # Start at mu=0.10 (not near zero) to avoid the Bessel crossover regime:
    # at mu=0 the QS distribution gives <r> ~ D^{1/4} (Bessel process), not D^{1/2}.
    # The limit cycle exponent 1/2 is only clean once mu >> sqrt(D) ~ 0.022 at D=5e-4.
    mu_vals = np.linspace(0.10, 0.8, 12)
...
""")
    ...
    if show_plots:
        ...
        ax = axes[1]
        # Right panel: show the EXPONENT ratio, not the value ratio.
        # S_SN/S_TC = κ^{2/3}/κ^{1/3} = κ^{1/3} — that's a value ratio, not the
        # exponent ratio. The correct display is the log-log slopes themselves
        # and their ratio p_SN/p_TC = 2, shown as a bar chart.
        slope_tc_meas, _ = fit_loglog(kappa_vals, severity_tc(kappa_vals))
        slope_sn_meas, _ = fit_loglog(kappa_vals, severity_sn(kappa_vals))
        ratio_meas = slope_sn_meas / slope_tc_meas

        x_pos = [1, 2]
        theory_exps = [1/3, 2/3]
        meas_exps   = [slope_tc_meas, slope_sn_meas]
        labels      = ["TC  (p=1/3)", "SN  (p=2/3)"]
        cols        = ["C0", "C3"]

        ax.bar([p - 0.2 for p in x_pos], theory_exps, width=0.35,
               label="Theory (exact)", color=cols, alpha=0.45)
        ax.bar([p + 0.2 for p in x_pos], meas_exps, width=0.35,
               label="Measured log-log slope", color=cols, alpha=0.9)
        ax.set_xticks(x_pos)
        ax.set_xticklabels(labels)
        ax.set_ylabel("Severity exponent  p")
        ax.set_title(
            fr"Exponent Ratio: $p_{{SN}}/p_{{TC}}$ = {ratio_meas:.4f}  (theory 2.0000)"
        )
        ax.legend(fontsize=9)
        ax.grid(True, alpha=0.3, axis="y")
        # Annotate ratio
        ax.text(0.5, 0.08, fr"$p_{{SN}}/p_{{TC}} = {ratio_meas:.4f}$",
                transform=ax.transAxes, ha="center", fontsize=11,
                color="k", bbox=dict(boxstyle="round,pad=0.3", fc="lightyellow", ec="gray"))
...
def parse_args() -> argparse.Namespace:
    ...
    # When run inside Jupyter, sys.argv contains kernel arguments that
    # argparse cannot handle (e.g. '-f kernel-xxx.json').
    # Detect this and fall back to defaults.
    in_jupyter = any(
        arg.endswith(".json") or "ipykernel" in arg or "jupyter" in arg
        for arg in sys.argv[1:]
    )
    return p.parse_args([] if in_jupyter else None)
...
def main() -> None:
    ...
    # ── Notebook users ──────────────────────────────────────────────────────
    # argparse defaults to running all sections with plots enabled.
    # To run a specific section from a notebook cell, call directly:
    #   section_2_noise_scaling(show_plots=True)
    #   section_8_pareto(show_plots=True)
    # Or override args before calling main():
    #   import sys; sys.argv = ['rst_core_derivations.py', '--section', '2']
    # ────────────────────────────────────────────────────────────────────────
